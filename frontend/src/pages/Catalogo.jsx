...
export default function Catalogo() {
  ...
  // 2. Nueva función para dar Feedback visual
  const handleAgregar = (product) => {
    addToCart(product);
    
    // Notificación rápida que aparece arriba a la derecha
    Swal.fire({
      toast: true,
      position: 'top-end',
      icon: 'success',
        ...
        {products.map(product => (
          <div key={product._id} style={cardStyle}>
            ...
            <div style={infoStyle}>
              ...
              <button 
                // 3. Cambiamos el onClick para usar la nueva función
...
const cardStyle = { backgroundColor: '#fff', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 8px 20px rgba(0,0,0,0.08)', border: '1px solid #eee', transition: 'transform 0.3s ease' };
...
const badgeAgotado = { position: 'absolute', bottom: '10px', right: '10px', backgroundColor: '#333', color: 'white', padding: '5px 12px', borderRadius: '8px', fontSize: '0.75rem' };
...
const btnStyle = { width: '100%', padding: '12px', border: 'none', borderRadius: '12px', color: 'white', fontWeight: 'bold', fontSize: '1rem', marginTop: '10px', transition: '0.3s' };